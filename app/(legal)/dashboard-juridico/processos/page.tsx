...
function MovementsView() {
  return <LegalPage>
    ...
    <section className="py-8">
      ...
      <div className="border-t border-white/[0.07]">{movements.map(({ icon: Icon, ...item }) => <div key={`${item.title}-${item.caseName}`} className="grid gap-4 border-b border-white/[0.06] py-5 sm:grid-cols-[32px_minmax(0,1fr)_80px_70px] sm:items-center"><span className="grid size-8 place-items-center text-od-text-3/65"><Icon size={15} /></span><div><div className="flex flex-wrap items-center gap-2"><p className="text-[12px] font-medium text-white/72">{item.title}</p><StatusTag tone={item.tone}>{item.court}</StatusTag></div><p className="mt-1.5 text-[11px] text-white/42">{item.caseName}</p><p className="mt-1 text-[10px] text-white/27">{item.detail}</p></div><span className="text-[10px] text-white/30">{item.time}</span><button type="button" className="text-left text-[10px] font-medium text-od-accent/65 hover:text-od-accent sm:text-right">Revisar</button></div>)}</div>
    </section>
  </LegalPage>;
}
