  ...
  return (
    <section className="panel space-y-4 p-5 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-black text-ink">Qualidade do anúncio</h2>
        ...
        <span className={"rounded-full px-3 py-1.5 text-sm font-black " + scoreColor(property.listing_quality_score)}>
      ...
      {canManage && (
        <form action={recalculateListingQuality}>
          ...
          <PendingButton className="btn-soft" pendingLabel="Calculando">
      ...
      <div className="border-t border-line pt-4">
        <h3 className="text-sm font-black text-ink">Checklist documental</h3>
        {documents.length === 0 ? (
          <p className="mt-2 text-sm font-medium text-ink-muted">Nenhum item no checklist ainda.</p>
        ) : (
          <ul className="mt-2 space-y-2">
            {documents.map((doc) => (
              <li key={doc.id} className="flex items-center justify-between gap-2 rounded-md border border-line px-3 py-2">
                ...
                <div className="flex items-center gap-2">
                  {canManage ? (
                    <form action={updateDocumentChecklistStatus} className="flex items-center gap-1.5">
                      ...
                      <button type="submit" className="press-sm rounded-md border border-line bg-white px-2 py-1 text-[11px] font-bold text-ink-muted hover:bg-surface-2">
                        Salvar
                      </button>
                    </form>
                  ) : (
