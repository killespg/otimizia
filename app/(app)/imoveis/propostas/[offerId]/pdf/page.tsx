...
export default async function OfferPdfPage({ params }: { params: { offerId: string } }) {
  ...
  return (
    <div className="print-document mx-auto max-w-2xl space-y-6">
      ...
      <header className="border-b border-line pb-4">
        <p className="text-sm font-black text-brand-700">{org?.name ?? "Proposta"}</p>
        <h1 className="mt-2 text-2xl font-black text-ink">Proposta de {offer.status === "sent" || offer.status === "viewed" ? "compra" : "negociação"}</h1>
        <p className="mt-1 text-sm font-medium text-ink-muted">Gerada em {new Date(offer.created_at).toLocaleDateString("pt-BR")}</p>
      </header>

      <section>
        <h2 className="text-sm font-black uppercase tracking-wide text-ink-muted">Imóvel</h2>
        <p className="mt-1 text-lg font-black text-ink">{property?.title ?? "—"}</p>
        {address && <p className="text-sm text-ink-soft">{address}</p>}
      </section>

      <section>
        <h2 className="text-sm font-black uppercase tracking-wide text-ink-muted">Cliente</h2>
      ...
      <section className="rounded-lg border border-line p-4">
        <h2 className="text-sm font-black uppercase tracking-wide text-ink-muted">Condições propostas</h2>
        <dl className="mt-3 grid gap-3 sm:grid-cols-2">
          <div>
            <dt className="text-xs font-bold text-ink-muted">Valor</dt>
            <dd className="text-lg font-black text-ink">{centsToReais(offer.amount_cents)}</dd>
          </div>
      ...
      <Link href={`/imoveis/match/${offer.deal_id}`} className="nav-item print-hide inline-block text-sm font-black text-brand-700 hover:underline">
