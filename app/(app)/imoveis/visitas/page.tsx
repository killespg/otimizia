  ...
  return (
    <div className="max-w-3xl space-y-4 sm:space-y-5">
      <header className="enter">
        ...
        <h1 className="mt-2 text-[clamp(1.55rem,6vw,2.6rem)] font-black leading-[1.02] tracking-[-0.04em] text-ink">Visitas</h1>
        <p className="mt-2 text-sm font-medium leading-relaxed text-ink-soft">
          Solicitações, agenda e histórico — lembretes automáticos saem por WhatsApp (24h antes) e push (2h antes).
        </p>
      </header>

      {requested.length > 0 && (
        <section className="panel p-5 sm:p-6">
          <h2 className="mb-3 text-base font-black text-ink">Solicitações pendentes ({requested.length})</h2>
          <div className="space-y-3">
            {requested.map((visit) => (
              <div key={visit.id} className="rounded-lg border border-line p-4">
                <p className="text-sm font-black text-ink">{propertyById.get(visit.property_id)?.title ?? "Imóvel"}</p>
      ...
      <section className="panel p-5 sm:p-6">
        <h2 className="mb-3 text-base font-black text-ink">Agendadas ({scheduled.length})</h2>
        {scheduled.length === 0 ? (
                ...
                {canManage && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <form action={cancelVisit}>
                      <input type="hidden" name="visit_id" value={visit.id} />
                      <PendingButton className="press-sm rounded-md border border-line bg-white px-2.5 py-1 text-xs font-bold text-danger-600 hover:bg-danger-50" pendingLabel="...">
                    ...
                    <form action={completeVisit} className="flex flex-1 flex-wrap items-end gap-2">
                      ...
                      <PendingButton className="btn-soft shrink-0" pendingLabel="...">
                        Concluir
                      </PendingButton>
                    </form>
                  </div>
      ...
      {history.length > 0 && (
        <section className="panel p-5 sm:p-6">
          <h2 className="mb-3 text-base font-black text-ink">Histórico</h2>
