...
export function OffersSection({
  ...
  return (
    <section className="panel space-y-4 p-5 sm:p-6">
      <h2 className="text-base font-black text-ink">Propostas</h2>
  ...
  return (
    <div className="rounded-lg border border-line p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm font-black text-ink">{centsToReais(offer.amount_cents)}</p>
        <div className="flex items-center gap-2">
          <span className="tag bg-surface-2 text-ink-muted">{OFFER_STATUS_LABEL[offer.status]}</span>
          <Link href={`/imoveis/propostas/${offer.id}/pdf`} className="nav-item text-xs font-black text-brand-700 hover:underline">
            PDF
          </Link>
        </div>
      </div>
      ...
      {chain.map((counter) => (
        <div key={counter.id} className="mt-2 ml-4 border-l-2 border-line pl-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-sm font-black text-ink">Contraproposta: {centsToReais(counter.amount_cents)}</p>
