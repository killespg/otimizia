...
export default async function CalendarPage({
  ...
  const [orgRole, members, { data: taskRows }, { data: contactRows }] =
    await Promise.all([
  ...
  const canManageDeadlines =
    ...
    canManageLegal(currentMember?.job_role, isAdmin);
  ...
  const overdueTasks = tasksWithDate.filter(
    (task) => new Date(task.due_at as string) < now,
  );
  const overdueDeadlines = legalDeadlines.filter(
    (deadline) => new Date(deadline.due_at) < now,
  );
  const todayTasks = tasksWithDate.filter(
    (task) =>
      new Date(task.due_at as string) >= now &&
      new Date(task.due_at as string) <= endOfToday,
  );
  const todayDeadlines = legalDeadlines.filter(
    (deadline) =>
      new Date(deadline.due_at) >= now &&
      new Date(deadline.due_at) <= endOfToday,
  );
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <section className="grid grid-cols-3 gap-3 sm:gap-4">
        ...
        <StatCard
          label="Hoje"
          value={String(todayTasks.length + todayDeadlines.length)}
          icon={IconClock}
          tone="warning"
        />
        <StatCard
        ...
        <div className="grid grid-cols-7">
          {cells.map((day, index) => {
            ...
            const isToday = Boolean(
              day && isCurrentMonth && day === now.getDate(),
            );
            return (
              <div
                key={index}
                        ...
                        <Link
                          ...
                          className={
                            ...
                            (entry.kind === "task"
                              ...
                              : (PRIORITY_COLOR[entry.deadline.priority] ??
      ...
      <SectionCard
        ...
        actions={<Tag>{monthEntries.length}</Tag>}
      >
        {dayGroups.length === 0 ? (
          <EmptyState icon={IconCheckCircle} title="Nada agendado neste mês." />
        ) : (
                ...
                <ul className="mt-2 divide-y divide-line">
                  {entries.map((entry, entryIndex) =>
                    entry.kind === "task" ? (
                      ...
                      <li
                        key={`deadline-${entry.deadline.id}`}
                          ...
                          {canManageDeadlines && (
                            <form action={completeLegalDeadline}>
                              <input
                                type="hidden"
                                name="id"
                                value={entry.deadline.id}
                              />
                              <input
                                type="hidden"
                                name="case_id"
                                value={entry.deadline.case_id}
                              />
                              <PendingButton
