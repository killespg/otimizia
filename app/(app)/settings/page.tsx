...
export default async function SettingsPage({
  ...
  const notificationPrefs =
    notificationPrefsData as NotificationPreferences | null;
  ...
  return (
    <div className="settings-hub max-w-6xl space-y-4 sm:space-y-5">
      <PageHeader
        ...
        description="Ajuste o CRM para ficar com a sua cara: painel, métricas, widgets, nomes do workspace, conta e plano."
      />
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(20rem,0.75fr)]">
            ...
            >
              <form action={updateWorkspaceLabels} className="space-y-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <Field
                    name="contacts_label"
                    label="Pessoas ou base"
                    defaultValue={workspaceLabels.contacts}
                    maxLength={40}
                  />
                  <Field
                    name="pipeline_label"
                    label="Quadro principal"
                    defaultValue={workspaceLabels.pipeline}
                    maxLength={40}
                  />
                  <Field
                    name="deal_singular_label"
                    label="Item do quadro"
                    defaultValue={workspaceLabels.dealSingular}
                    maxLength={40}
                  />
                  <Field
                    name="value_label"
                    label="Valor acompanhado"
                    defaultValue={workspaceLabels.value}
                    maxLength={40}
                  />
                  <div className="sm:col-span-2">
                    <Field
                      name="followups_label"
                      label="Retornos e revisões"
                      defaultValue={workspaceLabels.followups}
                      maxLength={40}
                    />
                  </div>
                </div>
            ...
            >
              ...
              <label className="flex min-h-11 items-center gap-2.5 rounded-lg border border-line px-3 py-2 text-sm font-bold text-ink-soft">
                <input
                  ...
                  defaultChecked={
                    notificationPrefs?.daily_summary_email ?? true
                  }
                  className="h-4 w-4 shrink-0 rounded border-line text-brand-700 focus:ring-brand-600"
                />
        ...
        <div className="space-y-4">
          <SectionCard
            title="Conta"
            description="Dados de login e identificação."
          >
            ...
            >
              ...
              <p className="text-xs font-medium text-ink-muted">
                Você vai receber um e-mail de confirmação no endereço novo antes
                da troca valer.
              </p>
              <PendingButton className="btn-soft" pendingLabel="Salvando">
                Salvar e-mail
          ...
          {isFounder ? (
            <SectionCard
              ...
              description={
                <>
                  Sua conta usa o modo fundador do <BrandName />.
                </>
              }
            >
              <p className="text-sm font-medium text-ink-muted">
                Sua conta é especial: em vez de escolher uma área de atuação, o
                painel já vem pronto para acompanhar sua própria prospecção de
                clientes e as métricas do produto.
              </p>
            </SectionCard>
          ) : isOrgAdmin ? (
            <SectionCard
              title="Áreas de atuação"
              description="Escolha qual operação quer ver e alimentar agora."
            >
              <form action={updateProfessionTypes} className="space-y-3">
                <input
                  type="hidden"
                  name="active_profession_type"
                  value={preset.key}
                />
                  ...
                  {PROFESSION_OPTIONS.map((option) => (
                    ...
                    >
                      <input
                        ...
                        value={option.value}
                        defaultChecked={(
                          profile?.profession_types ?? [preset.key]
                ...
                <p className="text-xs font-medium leading-relaxed text-ink-muted">
                  Contatos, negócios, lembretes e assistente ficam separados por
                  área.
                </p>
            ...
            <SectionCard
              ...
              description="Definido pelo plano da sua empresa."
            >
              ...
              <p className="mt-2 text-xs font-medium leading-relaxed text-ink-muted">
                Sua conta usa o plano da empresa e fica limitada a esta área.
                Para acessar outras, é preciso um plano próprio.
              </p>
            </SectionCard>
          )}
            ...
            {isOrgAdmin && access.status === "past_due" && (
              <div className="space-y-3">
                <p className="text-sm font-bold text-danger-700">
                  Pagamento pendente — atualize a forma de pagamento para não
                  perder o acesso.
                </p>
                <form action="/api/billing/portal" method="POST">
                  <PendingButton className="btn-soft" pendingLabel="Abrindo">
          ...
          <SectionCard
            title="Zona de risco"
            description="Ações permanentes, sem volta."
            tone="danger"
          >
