...
export async function PipelineReportView({
  months,
  embedded = false,
  formAction,
  backHref = "/painel/funil",
}: {
  months?: string;
  embedded?: boolean;
  formAction?: string;
  ...
  const monthsBack = clampMonths(months);
  ...
  const createdLabel = isSeller ? "Vendas criadas" : isRealEstate ? "Atendimentos iniciados" : "Negócios criados";
  const wonLabel = isSeller || isRealEstate ? "Fechadas" : "Ganhos";
  const wonValueLabel = isSeller || isRealEstate ? "Valor fechado" : "Valor ganho";

  return (
    <div className={usesFlatSurface ? "w-full max-w-[1640px] space-y-6" : "max-w-5xl space-y-4 sm:space-y-5"}>
      {embedded ? (
        <div className="flex flex-wrap items-center justify-end gap-2">
          <MonthsForm monthsBack={monthsBack} flat={usesFlatSurface} action={formAction} keepTab={embedded} />
        </div>
      ) : (
          ...
          <div>
            ...
            <p className={usesFlatSurface ? "mt-2 max-w-xl text-sm leading-relaxed text-white/52" : "mt-2 max-w-xl text-sm font-semibold leading-relaxed text-ink-muted"}>
              {isRealEstate
                ...
                : `${isSeller ? "Vendas" : workspaceLabels.pipeline} por mês — criadas, fechadas, perdidas e taxa de conversão.`}
            </p>
          </div>

          <MonthsForm monthsBack={monthsBack} flat={usesFlatSurface} action={formAction} keepTab={false} />
        </header>
      )}

      <section className={usesFlatSurface ? "od-band grid sm:grid-cols-4" : "grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4"}>
        <MetricCard label={createdLabel} value={String(totals.created)} flat={usesFlatSurface} />
        <MetricCard label={wonLabel} value={String(totals.won)} flat={usesFlatSurface} />
        <MetricCard label={wonValueLabel} value={formatBRL(totals.wonValueCents)} flat={usesFlatSurface} />
        <MetricCard
        ...
        <table className={usesFlatSurface ? "w-full min-w-[680px] border-collapse text-sm" : "w-full min-w-[560px] border-collapse text-sm"}>
          <thead>
            <tr className={usesFlatSurface ? "text-left text-xs font-semibold uppercase tracking-[0.04em] text-od-text-3" : "text-left text-xs font-bold uppercase tracking-wide text-ink-muted"}>
              ...
              <th className={usesFlatSurface ? "border-b border-white/[0.08] px-4 py-3" : "border-b border-line pb-2"}>{isRealEstate ? "Iniciados" : "Criadas"}</th>
              <th className={usesFlatSurface ? "border-b border-white/[0.08] px-4 py-3" : "border-b border-line pb-2"}>{isSeller || isRealEstate ? "Fechadas" : "Ganhos"}</th>
              <th className={usesFlatSurface ? "border-b border-white/[0.08] px-4 py-3" : "border-b border-line pb-2"}>Perdidas</th>
              <th className={usesFlatSurface ? "border-b border-white/[0.08] px-4 py-3" : "border-b border-line pb-2"}>{wonValueLabel}</th>
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <form method="get" action={action} className="flex flex-wrap items-center gap-2">
        {keepTab ? <input type="hidden" name="tab" value="numeros" /> : null}
        ...
        <select
          ...
          className={flat ? "field min-h-11 !w-auto" : "field !w-auto"}
        >
          <option value="3">Últimos 3 meses</option>
          <option value="6">Últimos 6 meses</option>
          <option value="12">Últimos 12 meses</option>
        </select>
        <button type="submit" className={flat ? "min-h-11 rounded-md border border-white/[0.1] px-4 text-xs font-semibold text-white/64 hover:bg-white/[0.04] hover:text-white" : "btn-soft"}>
          Aplicar
        </button>
      </form>
      <a
        href={`/api/reports/deals?months=${monthsBack}`}
        className={flat ? "inline-flex min-h-11 items-center gap-1.5 rounded-md px-3 text-xs font-semibold text-od-text-2 hover:bg-white/[0.04] hover:text-od-text" : "btn-soft inline-flex items-center gap-1.5"}
      >
...
export function clampReportMonths(raw: string | undefined): number {
  const n = Number(raw);
