  ...
  return (
    <div className="mx-auto w-full max-w-[1640px] space-y-5">
      <header className="flex flex-col gap-4 border-b border-white/[0.08] pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs font-semibold text-violet-300">{isSeller ? "CRM" : isRealEstate ? "Imobiliário" : "Escritório"} / {workspaceLabels.pipeline}</p>
      ...
      <form id="new-deal" action={createDeal} className={usesFlatPipeline ? "scroll-mt-24 border-y border-white/[0.08] py-5" : "scroll-mt-24 border border-white/[0.09] bg-[#1e1d22] p-5"}>
        ...
        {usesFlatPipeline ? (
          <details className="mt-4 border-t border-white/[0.07] pt-3">
            <summary className="w-fit cursor-pointer text-xs font-semibold text-violet-300 hover:text-violet-200">Mais opções</summary>
            <OptionalDealFields />
          </details>
        ) : <OptionalDealFields />}
      </form>
