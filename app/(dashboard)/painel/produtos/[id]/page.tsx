      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="min-w-0 space-y-5">
          <section className="border border-white/[0.09] bg-[#1e1d22]/90">
            <header className="border-b border-white/[0.08] px-4 py-4"><h2 className="text-sm font-semibold text-white">Dados do produto</h2><p className="mt-1 text-xs text-white/42">Alterações futuras não modificam os pedidos já confirmados.</p></header>
          ...
          <section className="border border-white/[0.09] bg-[#1e1d22]/90">
            <header className="flex items-center justify-between gap-3 border-b border-white/[0.08] px-4 py-4"><div><h2 className="text-sm font-semibold text-white">Variações</h2><p className="mt-1 text-xs text-white/42">Tamanhos, cores, modelos, voltagens ou outras opções do mesmo produto.</p></div><Boxes size={18} className="text-violet-300" /></header>
            {variants.length ? <div className="divide-y divide-white/[0.08]">{variants.map((variant) => <div key={variant.id} className="grid gap-2 px-4 py-3 text-sm sm:grid-cols-[minmax(10rem,1fr)_8rem_8rem_7rem] sm:items-center"><div><strong className="font-semibold text-white/82">{variant.name}</strong><p className="mt-1 text-xs text-white/38">{Object.entries(variant.attributes).map(([key, value]) => `${key}: ${value}`).join(" · ") || "Sem atributos extras"}</p></div><span className="text-xs text-white/52"><span className="mb-1 block text-[11px] font-medium text-white/50 sm:hidden">SKU</span>{variant.sku || "Sem SKU"}</span><span className="font-medium tabular-nums text-white/72"><span className="mb-1 block text-[11px] font-medium text-white/50 sm:hidden">Preço</span>{money(variant.price_cents ?? product.base_price_cents)}</span><span className="font-semibold tabular-nums text-white/72"><span className="mb-1 block text-[11px] font-medium text-white/50 sm:hidden">Estoque</span>{variant.stock_quantity - variant.reserved_quantity} un.</span></div>)}</div> : <p className="px-4 py-6 text-sm text-white/42">Nenhuma variação cadastrada. O produto usa o estoque principal.</p>}
            <details className="group border-t border-white/[0.08] p-4">
              <summary className="flex min-h-11 cursor-pointer list-none items-center gap-2 text-sm font-semibold text-violet-300"><Plus size={15} /> Adicionar variação</summary>
          ...
          <section id="ajustar-estoque" className="scroll-mt-24 border border-white/[0.09] bg-[#1e1d22]/90">
            <header className="border-b border-white/[0.08] px-4 py-4"><h2 className="text-sm font-semibold text-white">Movimentar estoque</h2><p className="mt-1 text-xs text-white/42">Use quantidade positiva para entrada e negativa para saída ou correção.</p></header>
          ...
          <section className="border border-white/[0.09] bg-[#1e1d22]/90">
            ...
            {movements.length ? <div className="divide-y divide-white/[0.07]">{movements.map((movement) => <div key={movement.id} className="grid gap-2 px-4 py-3 text-xs sm:grid-cols-[8rem_minmax(10rem,1fr)_6rem_7rem]"><span className="text-white/42"><span className="mr-2 font-medium text-white/50 sm:hidden">Data</span>{date(movement.created_at)}</span><span className="text-white/62"><span className="mr-2 font-medium text-white/50 sm:hidden">Motivo</span>{movement.reason || movement.movement_type}</span><strong className={movement.quantity_delta > 0 ? "text-emerald-300" : "text-[#fb7767]"}><span className="mr-2 font-medium text-white/50 sm:hidden">Movimento</span>{movement.quantity_delta > 0 ? "+" : ""}{movement.quantity_delta}</strong><span className="tabular-nums text-white/54 sm:text-right"><span className="mr-2 font-medium text-white/50 sm:hidden">Saldo</span>{movement.balance_after}</span></div>)}</div> : <p className="px-4 py-6 text-sm text-white/42">Nenhuma movimentação registrada.</p>}
          </section>
        </div>
...
function Metric({ label, value }: { label: string; value: string }) { return <div className="flex items-center justify-between gap-4 py-3"><dt className="text-white/46">{label}</dt><dd className="font-semibold tabular-nums text-white/82">{value}</dd></div>; }
