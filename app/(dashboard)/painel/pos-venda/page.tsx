          ...
          <section className="overflow-hidden panel">
            ...
            <div className="divide-y divide-white/[0.08]">{filteredWarranties.map((warranty) => {
              ...
              return <Link key={warranty.id} href={`/painel/pos-venda?warranty=${warranty.id}#novo-chamado`} className="group grid min-h-16 gap-2 px-4 py-3 hover:bg-white/[0.025] lg:grid-cols-[minmax(13rem,1.3fr)_minmax(10rem,1fr)_8rem_8rem_3rem] lg:items-center">
                ...
                <span className="text-sm font-semibold tabular-nums text-white/62"><span className="mb-1 block text-xs font-medium text-od-text-3 lg:hidden">Chamados</span>{claimCountByWarranty.get(warranty.id) ?? 0}</span>
                <ChevronRight size={15} className="hidden text-od-text-3 group-hover:text-od-text lg:block" />
              </Link>;
            })}</div>
          </section>

          <section id="novo-chamado" className="scroll-mt-24 border border-od-accent/20 bg-od-surface">
