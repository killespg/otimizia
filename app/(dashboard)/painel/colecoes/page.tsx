    ...
    <div className="mx-auto w-full max-w-[1500px] space-y-5">
      ...
      {collections.length === 0 ? <SellerEmptyState title="Nenhuma coleção criada" description="Coleções são opcionais. Use-as para moda, campanhas sazonais ou catálogos por temporada." action={<Link href="#nova" className="btn"><Plus size={16} /> Criar coleção</Link>} /> : (
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {collections.map((collection) => <article id={collection.id} key={collection.id} className="scroll-mt-24 rounded-[var(--radius-panel)] border border-white/[0.08] bg-[#1e1d22]/82 p-5">
            <div className="flex items-start justify-between gap-4"><span className="grid size-9 place-items-center rounded-[var(--radius-inner)] border border-od-accent/20 bg-od-accent/[0.06] text-od-text-2"><Layers3 size={17} /></span><SellerStatus tone={collection.status === "active" ? "success" : collection.status === "draft" ? "warning" : "neutral"}>{collection.status === "active" ? "Ativa" : collection.status === "draft" ? "Rascunho" : "Arquivada"}</SellerStatus></div>
            <h2 className="mt-5 text-base font-semibold text-white/86">{collection.name}</h2>
            <p className="mt-2 min-h-11 text-xs leading-relaxed text-od-text-3">{collection.description || "Sem descrição."}</p>
      ...
      <section id="trocar" className="scroll-mt-24 rounded-[var(--radius-panel)] border border-od-accent/20 bg-[#1e1d22]/90">
