...
import { isLegalCaseUuid } from "@/lib/law/legal-case-path";
...
import { LegalCaseDeadlineList } from "@/components/legal/legal-case-deadline-list";
...
export default async function LegalCasePage(props: { params: Promise<{ id: string }> }) {
  ...
  const lookup = supabase.from("legal_cases").select("*, contacts(name,phone,email)").eq("org_id", orgId);
  const { data } = isLegalCaseUuid(params.id)
    ? await lookup.eq("id", params.id).maybeSingle()
    : await lookup.eq("slug", params.id).maybeSingle();
  if (!data) notFound();
  if (isLegalCaseUuid(params.id) && data.slug && data.slug !== params.id) {
    permanentRedirect(`/juridico/processos/${data.slug}`);
  }
  ...
  return <div className="ui-page">
    <Link href="/juridico/processos" className="inline-flex items-center gap-2 text-xs font-semibold text-od-text-2 hover:text-od-text"><IconArrowRight className="h-3.5 w-3.5 rotate-180"/>Voltar para processos</Link>
    <header><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-semibold text-od-text-2">{legalCase.area??"Caso jurídico"}</p><h1 className="mt-2 text-od-title text-white">{legalCase.title}</h1><div className="mt-4 flex flex-wrap gap-2"><span className="rounded-[var(--radius-round)] bg-white/[0.06] px-2 py-1 text-xs font-semibold text-od-text">{LEGAL_CASE_STATUS[legalCase.status]}</span>{legalCase.case_number&&<span className="rounded-[var(--radius-round)] border border-white/[0.09] px-2 py-1 font-mono text-xs text-white/52">{legalCase.case_number}</span>}{legalCase.court&&<span className="rounded-[var(--radius-round)] border border-white/[0.09] px-2 py-1 text-xs text-white/52">{legalCase.court}</span>}</div></div>{canManage&&<form action={updateLegalCaseStatus} className="flex min-w-56 gap-2"><input type="hidden" name="id" value={legalCase.id}/><label className="sr-only" htmlFor="case-status">Situação</label><select id="case-status" name="status" defaultValue={legalCase.status} className="field">{Object.entries(LEGAL_CASE_STATUS).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select><PendingButton className="btn" pendingLabel="Salvando">Salvar</PendingButton></form>}</div></header>
    <section className="ui-metric-band sm:grid-cols-2 xl:grid-cols-4"><Metric label="Prazos pendentes" value={String(pending.length)} icon={IconClock}/><Metric label="Movimentações" value={String(events.length)} icon={IconArrowRight}/><Metric label="Documentos" value={String(documents.length)} icon={IconPaperclip}/>{canViewFinance(jobRole,isAdmin)&&<Metric label="Honorários em aberto" value={formatBRL(open)} icon={IconWallet}/>}</section>
