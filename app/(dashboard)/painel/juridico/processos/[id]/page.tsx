...
export default async function LegalCasePage(props: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto w-full max-w-[1640px] space-y-5">
    <Link href="/painel/juridico/processos" className="inline-flex items-center gap-2 text-[11px] font-semibold text-violet-300 hover:text-violet-200"><IconArrowRight className="h-3.5 w-3.5 rotate-180"/>Voltar para processos</Link>
    <header className="border-b border-white/[0.08] pb-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-semibold text-violet-300">{legalCase.area??"Caso jurídico"}</p><h1 className="mt-2 text-od-title text-white">{legalCase.title}</h1><div className="mt-4 flex flex-wrap gap-2"><span className="rounded-md bg-violet-400/10 px-2 py-1 text-xs font-semibold text-violet-200">{LEGAL_CASE_STATUS[legalCase.status]}</span>{legalCase.case_number&&<span className="rounded-md border border-white/[0.09] px-2 py-1 font-mono text-xs text-white/52">{legalCase.case_number}</span>}{legalCase.court&&<span className="rounded-md border border-white/[0.09] px-2 py-1 text-xs text-white/52">{legalCase.court}</span>}</div></div>{canManage&&<form action={updateLegalCaseStatus} className="flex min-w-56 gap-2"><input type="hidden" name="id" value={legalCase.id}/><label className="sr-only" htmlFor="case-status">Situação</label><select id="case-status" name="status" defaultValue={legalCase.status} className="field">{Object.entries(LEGAL_CASE_STATUS).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select><PendingButton className="btn" pendingLabel="Salvando">Salvar</PendingButton></form>}</div></header>
    <section className="grid border-y border-white/[0.08] sm:grid-cols-2 xl:grid-cols-4"><Metric label="Prazos pendentes" value={String(pending.length)} icon={IconClock}/><Metric label="Movimentações" value={String(events.length)} icon={IconArrowRight}/><Metric label="Documentos" value={String(documents.length)} icon={IconPaperclip}/>{canViewFinance(jobRole,isAdmin)&&<Metric label="Honorários em aberto" value={formatBRL(open)} icon={IconWallet}/>}</section>
...
function Metric({label,value,icon:Icon}:{label:string;value:string;icon:(p:{className?:string})=>React.ReactElement}){return <article className="flex items-center gap-3 border-b border-white/[0.08] px-4 py-4 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0"><Icon className="h-4 w-4 text-violet-300"/><div><p className="text-xs text-white/50">{label}</p><p className="mt-1 text-od-subtitle font-bold tracking-[-.02em] text-white">{value}</p></div></article>}
...
function SectionTitle({title,count}:{title:string;count:number}){return <div className="flex items-center justify-between border-b border-white/[0.08] px-5 py-4"><h2 className="text-base font-semibold text-white">{title}</h2><span className="text-xs text-white/48">{count}</span></div>}
...
function VisibilityToggle({action,id,caseId,visible}:{action:(formData:FormData)=>Promise<void>;id:string;caseId:string;visible:boolean}){return <form action={action}><input type="hidden" name="id" value={id}/><input type="hidden" name="case_id" value={caseId}/><input type="hidden" name="client_visible" value={String(visible)}/><button type="submit" className={"text-[10px] font-black "+(visible?"text-brand-700":"text-ink-muted")}>{visible?"Visível ao cliente":"Marcar visível ao cliente"}</button></form>}
function SignatureStatus({signature}:{signature:LegalDocumentSignature}){
  ...
  return <div className="flex flex-col items-end gap-1">
    ...
    {signature.status==="signed"&&signature.signed_file_url&&<a href={signature.signed_file_url} target="_blank" rel="noreferrer" className="text-[10px] font-black text-brand-700">Ver assinado</a>}
  </div>;
}
