...
function Metric({ label, value, danger, success }: { label: string; value: string; danger?: boolean; success?: boolean }) { return <div className="border-r border-white/[0.08] px-4 py-5 last:border-r-0"><strong className={`text-[25px] font-bold tracking-[-.03em] ${danger ? "text-[#fb7767]" : success ? "text-emerald-300" : "text-white"}`}>{value}</strong><p className="mt-1 text-xs text-od-text-3">{label}</p></div>; }
