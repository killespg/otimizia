...
export default async function DocumentsPage() {
  ...
  return <div className="mx-auto w-full max-w-[1640px]">
    ...
    <section className="grid grid-cols-1 border-b border-white/[0.08] sm:grid-cols-2 lg:grid-cols-4"><Metric label="Documentos ativos" value={String(docs.filter((item) => item.status !== "archived").length)}/><Metric label="Aguardando revisão" value={String(reviewCount)} danger/><Metric label="Para assinatura" value={String(awaitingSignature)}/><Metric label="Protocolados no mês" value={String(filedThisMonth)} success/></section>
