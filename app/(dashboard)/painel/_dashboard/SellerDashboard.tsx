      ...
      <div className={`grid sm:grid-cols-2 ${columnClass}`}>
        {attentionItems.map((item) => { const Icon = item.icon; return (
          <Link key={item.label} href={item.href} className="group flex min-h-20 items-center gap-3 border-t border-white/[0.08] px-4 hover:bg-white/[0.025] sm:[&:nth-child(2n)]:border-l lg:border-l lg:border-t-0">
            <span className={`grid size-8 shrink-0 place-items-center rounded border ${item.alert ? "border-[#fb7767]/25 bg-[#fb7767]/10 text-[#fca79b]" : "border-white/[0.10] bg-white/[0.05] text-od-text-2"}`}><Icon size={16} /></span>
            <span className="min-w-0 flex-1"><span className="block text-xs font-medium text-white/58">{item.label}</span><span className="mt-1 flex min-w-0 items-baseline gap-2"><strong className="text-lg font-semibold tabular-nums text-white">{item.value}</strong><span className="truncate text-xs text-od-text-3">{item.note}</span></span></span>
            <ArrowRight size={14} className="ml-auto shrink-0 text-od-text-3 group-hover:text-od-text" />
          </Link>
        ); })}
      </div>
    </section>
        ...
        return (
          ...
          >
            ...
            <div className="min-w-0">
              <p data-dashboard-metric-label={metric.metricKey} className="text-xs font-medium text-od-text-3">{metric.label}</p>
              <p className="mt-2 truncate text-2xl font-bold tracking-[-0.03em] text-white">{metric.value}</p>
              {metric.delta || metric.compare ? <p className="mt-2 truncate text-xs text-od-text-3">{metric.delta ? <span className="mr-1.5 font-semibold text-od-text-2">{metric.delta}</span> : null}{metric.compare}</p> : null}
            </div>
          </article>
        );
      })}
      ...
      <dl className="mt-3 divide-y divide-white/[0.07]">
        {items.map((item) => (
          <div key={item.label} className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-4 gap-y-1 py-3 first:pt-0 last:pb-0">
            <dt className="text-xs text-od-text-3">{item.label}</dt>
            <dd className="max-w-44 truncate text-right text-sm font-semibold text-white/82" title={item.value}>{item.value}</dd>
            <dd className="col-span-2 text-xs leading-relaxed text-od-text-3">{item.note}</dd>
          </div>
        ))}
      </dl>
    </div>
    ...
    <section className="grid items-stretch gap-4 xl:grid-cols-[minmax(0,2fr)_minmax(18rem,1fr)] xl:gap-6">
      <div data-dashboard-card className="overflow-hidden panel">
        ...
        {tasks.length === 0 ? (
          <div className="flex min-h-28 items-center gap-3 px-4 py-5">
            ...
            <div><p className="text-sm font-semibold text-white">Tudo em dia por aqui.</p><p className="mt-1 text-xs text-od-text-3">Seus próximos lembretes aparecerão nesta fila.</p></div>
          </div>
        ) : (
          <div>
            <div className="hidden grid-cols-[7rem_minmax(0,1.2fr)_minmax(9rem,.8fr)_7rem_1rem] gap-3 border-b border-white/[0.08] px-4 py-2 text-xs font-semibold uppercase tracking-[0.04em] text-od-text-3 md:grid">
            ...
            {tasks.map((task) => {
              ...
              return (
                <Link key={task.id} href="/tarefas" className="group grid min-h-16 gap-2 border-b border-white/[0.08] px-4 py-3 last:border-b-0 hover:bg-white/[0.025] md:grid-cols-[7rem_minmax(0,1.2fr)_minmax(9rem,.8fr)_7rem_1rem] md:items-center md:gap-3">
                  ...
                  <span className="text-xs text-od-text-3">{task.due_at ? formatDate(task.due_at) : "Sem data"}</span>
                  <Check size={15} className="hidden text-od-text-3 group-hover:text-od-text md:block" />
                </Link>
              );
            })}
          </div>
      ...
      <form id="novo-lembrete" action={createTask} data-dashboard-card className="panel p-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div><h2 className="text-sm font-semibold text-white">Criar lembrete rápido</h2><p className="mt-1 text-xs text-od-text-3">Registre o próximo contato sem sair do painel.</p></div>
        </div>
          ...
          {recent.map((deal) => {
            ...
            return (
              <Link href="/funil" key={deal.id} className="grid gap-2 border-b border-white/[0.07] px-5 py-4 last:border-b-0 hover:bg-white/[0.025] sm:grid-cols-[minmax(0,1.2fr)_9rem_8rem_8rem] sm:items-center sm:gap-3">
                ...
                <span className="text-xs text-od-text-3">{formatDate(deal.created_at)}</span>
              </Link>
            );
          })}
        </div>
    ...
    <section data-dashboard-card className="h-full panel p-4 sm:p-6">
      ...
      {hasRevenueData ? (
        <>
          ...
          <p className="mt-1 text-xs text-od-text-3">ganhos registrados no mês</p>
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0 px-3 py-3 sm:py-4"><p className="truncate text-xs text-od-text-3">{label}</p><p className="mt-2 truncate text-sm font-semibold text-white/76">{value}</p></div>;
}
      ...
      <div className="flex items-center justify-between gap-3 border-b border-white/[0.08] px-4 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-od-text-3">Ações rápidas com o contexto da sua carteira.</p>
          </div>
        </div>
      ...
      <div className="divide-y divide-white/[0.07] xl:grid xl:grid-cols-3 xl:divide-x xl:divide-y-0">
        {[
          ...
          >
            ...
            <ArrowRight size={14} className="text-od-text-3 transition-transform group-hover:translate-x-0.5 group-hover:text-od-text" />
          </Link>
        ))}
      </div>
...
function SellerFounderMetrics({ metrics }: { metrics: DevMetrics }) {
  ...
  return <section className="grid border-y border-od-border bg-od-muted-surface sm:grid-cols-4">{items.map(([label, value]) => <div key={label} className="border-b border-od-border px-4 py-3 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0"><p className="text-xs text-od-text-3">{label}</p><p className="mt-1 text-lg font-semibold text-white">{value}</p></div>)}</section>;
}
