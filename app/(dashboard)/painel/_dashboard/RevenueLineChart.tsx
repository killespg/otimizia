...
export function RevenueLineChart({ series, dark = false }: { series: Point[]; dark?: boolean }) {
  ...
  return (
    <div
      className="relative mt-4 h-[190px] overflow-hidden rounded-md bg-surface-2 sm:mt-5 sm:h-[280px]"
      ...
      >
        <defs>
          <linearGradient id={gradientId} x1="0" x2="0" y1="0" y2="1">
            <stop offset="0%" style={{ stopColor: "var(--dashboard-accent, #4E7FBF)" }} stopOpacity="0.2" />
            <stop offset="100%" style={{ stopColor: "var(--dashboard-accent, #4E7FBF)" }} stopOpacity="0" />
          </linearGradient>
        </defs>
        ...
        {gridSteps.map((step) => {
          ...
          return (
            <line
              ...
              y1={lineY}
              y2={lineY}
              stroke={dark ? "rgba(255,255,255,.08)" : "#dbe2ef"}
              strokeWidth="1"
            />
          );
        })}
        <g className="hidden sm:block">
          {gridSteps.map((step) => (
            <text
              ...
              textAnchor="end"
              fill={dark ? "rgba(255,255,255,.36)" : "#60708f"}
              fontSize="13"
              fontWeight="700"
            >
        ...
        {linePath && (
          <path
            ...
            d={linePath}
            fill="none"
            style={{ stroke: "var(--dashboard-accent-strong, #2F6FCC)" }}
            strokeLinecap="round"
        ...
        {n > 0 && (
          <circle
            ...
            r="4"
            style={{ fill: "var(--dashboard-accent-strong, #2F6FCC)" }}
            stroke="#fff"
            strokeWidth="2"
          />
        )}
        ...
        {displayed && (
          <line
            ...
            y1={TOP}
            y2={BOTTOM}
            style={{ stroke: "var(--dashboard-accent, #4E7FBF)" }}
            strokeWidth="1"
            strokeDasharray="4 4"
