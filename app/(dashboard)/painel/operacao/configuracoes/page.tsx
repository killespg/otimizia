...
export default async function SellerOperationSettingsPage({ searchParams }: { searchParams: Promise<{ salvo?: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1320px] space-y-6">
      <SellerPageHeader eyebrow="Gestão / Configurar operação" description="Escolha os controles que correspondem ao que você vende. O CRM continua simples e mostra apenas os módulos que fazem sentido." />
      {salvo === "1" ? <div role="status" className="flex min-h-12 items-center gap-3 rounded-[var(--radius-panel)] border border-emerald-400/25 bg-emerald-400/[0.06] px-4 text-sm text-emerald-300"><CheckCircle2 size={17} /><span>Configuração salva. Os módulos da operação já foram atualizados.</span></div> : null}
