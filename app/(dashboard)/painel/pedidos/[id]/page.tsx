...
export default async function SellerOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1450px] space-y-5">
      <Link href="/pedidos" className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-white/52 hover:text-white"><ArrowLeft size={15} /> Voltar aos pedidos</Link>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <aside className="space-y-4">
          ...
          {contact ? <section className="rounded-[var(--radius-panel)] border border-white/[0.09] bg-[#1e1d22]/90 p-4"><h2 className="text-sm font-semibold text-white">Cliente</h2><p className="mt-3 text-sm font-semibold text-white/76">{contact.name}</p><p className="mt-1 text-xs text-od-text-3">{contact.phone || contact.email || "Sem contato informado"}</p><Link href={`/contatos/${contact.id}`} className="mt-3 inline-flex min-h-11 items-center text-xs font-semibold text-od-text-2">Abrir cliente</Link></section> : null}
        </aside>
      </div>
    </div>
  );
