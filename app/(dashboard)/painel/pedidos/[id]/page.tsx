  ...
  return (
    <div className="mx-auto w-full max-w-[1450px] space-y-5">
      ...
      <section className="overflow-hidden border border-white/[0.09] bg-[#1e1d22]/90">
        <header className="border-b border-white/[0.08] px-4 py-4"><h2 className="text-sm font-semibold text-white">Itens do pedido</h2><p className="mt-1 text-xs text-white/42">Os nomes, preços e garantias abaixo são a fotografia do momento da venda.</p></header>
        <div className="hidden grid-cols-[minmax(14rem,1.5fr)_7rem_8rem_8rem_9rem] border-b border-white/[0.08] px-4 py-2 text-xs font-semibold text-white/38 lg:grid"><span>Produto</span><span>Quantidade</span><span>Preço</span><span>Subtotal</span><span>Garantia</span></div>
        <div className="divide-y divide-white/[0.08]">{items.map((item) => {
          ...
          return <div key={item.id} className="grid gap-2 px-4 py-4 lg:grid-cols-[minmax(14rem,1.5fr)_7rem_8rem_8rem_9rem] lg:items-center">
            <div><strong className="block text-sm font-semibold text-white/82">{item.product_name_snapshot}</strong><p className="mt-1 text-xs text-white/38">{[item.variant_snapshot, item.sku_snapshot, item.serial_number ? `Série ${item.serial_number}` : null].filter(Boolean).join(" · ") || "Produto principal"}</p>{item.customization_notes ? <p className="mt-2 border-l-2 border-violet-400/45 pl-2 text-xs leading-5 text-white/56">{item.customization_notes}</p> : null}<p className="mt-1.5 text-[11px] text-white/42">{[item.promised_on ? `Entrega prometida ${date(item.promised_on)}` : null, item.reorder_due_on ? `Recompra sugerida ${date(item.reorder_due_on)}` : null, item.commission_percent > 0 ? `Comissão ${item.commission_percent.toLocaleString("pt-BR")}% · ${money(item.commission_cents)}` : null].filter(Boolean).join(" · ")}</p></div>
            <span className="text-sm tabular-nums text-white/62"><span className="mb-1 block text-[11px] font-medium text-white/50 lg:hidden">Quantidade</span>{item.quantity}</span><span className="text-sm tabular-nums text-white/62"><span className="mb-1 block text-[11px] font-medium text-white/50 lg:hidden">Preço</span>{money(item.unit_price_cents)}</span><strong className="text-sm tabular-nums text-white/78"><span className="mb-1 block text-[11px] font-medium text-white/50 lg:hidden">Subtotal</span>{money(item.line_total_cents)}</strong>
            <span><span className="mb-1 block text-[11px] font-medium text-white/50 lg:hidden">Garantia</span>{warranty ? <Link href={`/painel/pos-venda?warranty=${warranty.id}#novo-chamado`}><SellerStatus tone={expired ? "danger" : "success"}>{expired ? "Vencida" : `Até ${date(warranty.expires_on)}`}</SellerStatus></Link> : <span className="text-xs text-white/50">Sem garantia</span>}</span>
          </div>;
        })}</div>
      </section>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <section className="border border-white/[0.09] bg-[#1e1d22]/90">
          <header className="border-b border-white/[0.08] px-4 py-4"><h2 className="text-sm font-semibold text-white">Atualizar andamento</h2><p className="mt-1 text-xs text-white/42">Pagamento e entrega são independentes: um pedido pode estar pago e ainda em preparação.</p></header>
...
function Row({ label, value }: { label: string; value: string }) { return <div className="flex items-center justify-between gap-4 py-2"><dt className="text-white/44">{label}</dt><dd className="font-semibold tabular-nums text-white/72">{value}</dd></div>; }
function IconRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <div className="flex min-h-11 items-center gap-3 py-2"><span className="text-violet-300">{icon}</span><span className="min-w-0 flex-1 text-white/44">{label}</span><strong className="text-right text-xs font-semibold text-white/70">{value}</strong></div>; }
