...
describe("buildTodayQueue", () => {
  it("ordena por prioridade de sinal, não só por data", () => {
    const queue = buildTodayQueue({
      ...
      stalledDeals: [
        { id: "d2", title: "Negócio parado", contactId: "c1", contactName: "Ana", assigneeId: "u1", lastActivityAt: "2026-01-01T00:00:00.000Z", stage: "novo" },
      ],
  ...
  it("inclui o nome do contato no motivo de negócio parado quando disponível", () => {
    const queue = buildTodayQueue({
      now: NOW,
      overdueTasks: [],
      stalledDeals: [{ id: "d1", title: "Negócio", contactId: null, contactName: null, assigneeId: "u1", lastActivityAt: "2026-01-01T00:00:00.000Z", stage: "novo" }],
    });
