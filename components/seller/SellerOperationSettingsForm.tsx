...
export function SellerOperationSettingsForm(props: Props) {
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      {props.canEdit ? <div className="sticky bottom-[calc(64px+env(safe-area-inset-bottom))] z-[var(--z-sticky)] flex justify-end border-t border-od-border bg-od-bg py-3 md:bottom-0"><PendingButton className="btn" pendingLabel="Salvando configuração"><SlidersHorizontal size={15} /> Salvar configuração</PendingButton></div> : <p className="rounded-inner border border-amber-300/20 bg-amber-300/[0.04] p-4 text-sm text-amber-200">Você pode consultar esta configuração, mas somente a administração pode alterá-la.</p>}
    </form>
  );
}
