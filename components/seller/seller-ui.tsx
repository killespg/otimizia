...
export function SellerPageHeader({
  ...
  return (
    <header className="flex flex-col gap-4 border-b border-od-border pb-6 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-od-text-3">{description}</p>
      </div>
...
export function SellerSummaryStrip({ items }: { items: Array<{ label: string; value: ReactNode; tone?: "default" | "warning" | "success" }> }) {
  ...
  return (
    <section className={`grid border-y border-od-border bg-od-surface ${smallColumnClass} ${wideColumnClass}`}>
      {items.map((item, index) => (
        <div key={item.label} className={`border-od-border px-3 py-3 sm:px-4 ${index >= smallLastRowStart ? "border-b-0" : "border-b"} ${(index + 1) % smallColumns === 0 || index === items.length - 1 ? "border-r-0" : "border-r"} xl:border-b-0 ${index === items.length - 1 ? "xl:border-r-0" : "xl:border-r"}`}>
          <p className="text-xs text-od-text-3">{item.label}</p>
          <p className={`mt-1 text-lg font-semibold tabular-nums ${item.tone === "warning" ? "text-amber-300" : item.tone === "success" ? "text-emerald-300" : "text-od-text"}`}>{item.value}</p>
        </div>
      ))}
    </section>
  );
...
export function SellerEmptyState({ title, description, action, icon = "products", className = "" }: { title: string; description: string; action?: ReactNode; icon?: "products" | "box"; className?: string }) {
  ...
  return (
    <div className={`flex min-h-56 flex-col items-center justify-center border-y border-od-border px-5 py-10 text-center ${className}`}>
      ...
      <h2 className="mt-4 text-base font-semibold text-od-text">{title}</h2>
...
export function ProductThumb({ src, name, size = "md" }: { src?: string | null; name: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <span className={`grid shrink-0 place-items-center overflow-hidden border border-od-border bg-white/[0.035] text-od-text-3 ${sizeClass}`}>
      {src ? (
...
export function SellerStatus({ tone = "neutral", children }: { tone?: "neutral" | "violet" | "success" | "warning" | "danger"; children: ReactNode }) {
  const tones = {
    neutral: "border-od-border text-od-text-2",
