...
export function SellerSaleConfirmation({
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <section className="rounded-[var(--radius-panel)] border border-white/[0.09] bg-[#1e1d22]/92">
        ...
        <div className="relative border-b border-white/[0.08] p-4">
          ...
          {matches.length > 0 ? <div className="absolute left-4 right-4 top-[64px] z-[var(--z-dropdown)] max-w-2xl rounded-[var(--radius-inner)] border border-od-border bg-od-surface shadow-lg">
        ...
        {items.length === 0 ? <div className="flex min-h-44 flex-col items-center justify-center px-5 text-center"><Box size={23} className="text-od-text-3" /><p className="mt-3 text-sm font-semibold text-white/64">Nenhum item adicionado</p><p className="mt-1 text-xs text-od-text-3">A venda só será confirmada depois que os itens reais forem informados.</p></div> : <div>
          ...
          <div className="divide-y divide-white/[0.08]">{items.map((item) => <div key={item.key} className="grid gap-3 px-4 py-3 lg:grid-cols-[minmax(12rem,1.3fr)_7rem_7rem_8rem_7rem_2.5rem] lg:items-center">
            <div className="min-w-0"><div className="flex items-center gap-2"><strong className="truncate text-sm font-semibold text-white/82">{item.name}</strong>{item.quickProduct ? <span className="rounded-[var(--radius-control)] border border-od-accent/25 px-1.5 py-0.5 text-xs font-semibold text-od-text">Novo</span> : null}</div><p className="mt-1 truncate text-xs text-od-text-3">{[item.variantName, item.sku].filter(Boolean).join(" · ") || "Produto principal"}{item.available !== null ? ` · ${item.available} un. disponíveis` : ""}</p>{(item.requiresSerial || item.warrantyDays > 0) ? <input value={item.serialNumber} onChange={(event) => updateItem(item.key, { serialNumber: event.target.value })} placeholder="Número de série" className="field mt-2 h-11 min-h-11 text-xs" /> : null}{item.kind === "made_to_order" ? <div className="mt-2 grid gap-2 sm:grid-cols-2"><input value={item.customizationNotes} onChange={(event) => updateItem(item.key, { customizationNotes: event.target.value })} placeholder="Personalização / especificações" className="field h-11 min-h-11 text-xs" /><label><span className="sr-only">Entrega prometida</span><input value={item.promisedOn} onChange={(event) => updateItem(item.key, { promisedOn: event.target.value })} type="date" title="Entrega prometida" className="field h-11 min-h-11 text-xs" /></label></div> : null}{item.kind === "commercial_representative" ? <label className="mt-2 block"><span className="mb-1 block text-xs font-medium text-od-text-3">Comissão desta venda (%)</span><input value={item.commissionPercent} onChange={(event) => updateItem(item.key, { commissionPercent: Math.max(0, Math.min(100, Number(event.target.value) || 0)) })} type="number" min="0" max="100" step="0.01" className="field h-11 min-h-11 text-xs" /></label> : null}</div>
      ...
      <section className="grid rounded-[var(--radius-panel)] border border-white/[0.09] bg-[#1e1d22]/92 xl:grid-cols-[minmax(0,1fr)_330px]">
