    ...
    <form action={formAction} className="space-y-5">
      ...
      <section className="border border-white/[0.09] bg-[#1e1d22]/92">
        <header className="flex flex-col gap-3 border-b border-white/[0.08] p-4 sm:flex-row sm:items-center sm:justify-between">
          <div><h2 className="text-sm font-semibold text-white">Itens da venda</h2><p className="mt-1 text-xs text-white/42">Busque no catálogo ou cadastre o que está vendendo agora.</p></div>
        ...
        <div className="relative border-b border-white/[0.08] p-4">
          <label className="relative block max-w-2xl"><span className="sr-only">Buscar produto</span><Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-white/38" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar produto ou criar agora" className="field pl-10" /></label>
          {matches.length > 0 ? <div className="absolute left-4 right-4 top-[64px] z-[var(--z-dropdown)] max-w-2xl border border-white/[0.13] bg-[#151419] shadow-lg">
            {matches.map((option) => <button type="button" key={option.key} onClick={() => addCatalogOption(option)} className="grid min-h-14 w-full grid-cols-[minmax(0,1fr)_7rem_6rem] items-center gap-3 border-b border-white/[0.07] px-3 text-left last:border-b-0 hover:bg-white/[0.04]"><span className="min-w-0"><strong className="block truncate text-sm font-semibold text-white/78">{option.name}</strong><span className="mt-1 block truncate text-xs text-white/38">{option.variantName || option.sku || "Produto principal"}</span></span><span className="text-sm font-medium tabular-nums text-white/68">{formatMoney(option.price)}</span><span className={`text-right text-xs ${option.available !== null && option.available <= 0 ? "text-[#fb7767]" : "text-white/42"}`}>{option.available === null ? "Livre" : `${option.available} un.`}</span></button>)}
          </div> : null}
        </div>

        {showQuick ? <div className="border-b border-violet-400/18 bg-violet-400/[0.035] p-4">
          <div className="mb-4 flex items-center gap-3"><span className="grid size-8 place-items-center border border-violet-400/20 bg-violet-400/[0.08] text-violet-300"><PackagePlus size={16} /></span><div><h3 className="text-sm font-semibold text-white">Produto novo</h3><p className="mt-1 text-xs text-white/42">Preencha o essencial. Fotos e detalhes podem ser completados no catálogo.</p></div></div>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            {usesInventory ? <><QuickField label="Estoque inicial"><input value={quick.initialStock} onChange={(event) => setQuick({ ...quick, initialStock: Math.max(0, Number(event.target.value) || 0) })} type="number" min="0" className="field mt-1.5" /></QuickField><label className="flex min-h-11 items-center gap-2 self-end text-sm text-white/58"><input type="checkbox" checked={quick.trackStock} onChange={(event) => setQuick({ ...quick, trackStock: event.target.checked })} className="size-4 accent-violet-600" /> Controlar estoque</label></> : null}
            {usesWarranties ? <><QuickField label="Garantia em dias"><input value={quick.warrantyDays} onChange={(event) => setQuick({ ...quick, warrantyDays: Math.max(0, Number(event.target.value) || 0) })} type="number" min="0" max="3650" className="field mt-1.5" /></QuickField><QuickField label="Número de série"><input value={quick.serialNumber} onChange={(event) => setQuick({ ...quick, serialNumber: event.target.value })} placeholder="Opcional" className="field mt-1.5" /></QuickField><label className="flex min-h-11 items-center gap-2 self-end text-sm text-white/58"><input type="checkbox" checked={quick.requiresSerial} onChange={(event) => setQuick({ ...quick, requiresSerial: event.target.checked })} className="size-4 accent-violet-600" /> Exigir série</label></> : null}
          </div>
        ...
        {items.length === 0 ? <div className="flex min-h-44 flex-col items-center justify-center px-5 text-center"><Box size={23} className="text-white/28" /><p className="mt-3 text-sm font-semibold text-white/64">Nenhum item adicionado</p><p className="mt-1 text-xs text-white/38">A venda só será confirmada depois que os itens reais forem informados.</p></div> : <div>
          <div className="hidden grid-cols-[minmax(12rem,1.3fr)_7rem_7rem_8rem_7rem_2.5rem] border-b border-white/[0.08] px-4 py-2 text-xs font-semibold text-white/38 lg:grid"><span>Produto</span><span>Quantidade</span><span>Preço</span><span>Garantia</span><span>Subtotal</span><span /></div>
          <div className="divide-y divide-white/[0.08]">{items.map((item) => <div key={item.key} className="grid gap-3 px-4 py-3 lg:grid-cols-[minmax(12rem,1.3fr)_7rem_7rem_8rem_7rem_2.5rem] lg:items-center">
            <div className="min-w-0"><div className="flex items-center gap-2"><strong className="truncate text-sm font-semibold text-white/82">{item.name}</strong>{item.quickProduct ? <span className="border border-violet-400/25 px-1.5 py-0.5 text-[10px] font-semibold text-violet-200">Novo</span> : null}</div><p className="mt-1 truncate text-xs text-white/38">{[item.variantName, item.sku].filter(Boolean).join(" · ") || "Produto principal"}{item.available !== null ? ` · ${item.available} un. disponíveis` : ""}</p>{(item.requiresSerial || item.warrantyDays > 0) ? <input value={item.serialNumber} onChange={(event) => updateItem(item.key, { serialNumber: event.target.value })} placeholder="Número de série" className="field mt-2 h-9 min-h-9 text-xs" /> : null}{item.kind === "made_to_order" ? <div className="mt-2 grid gap-2 sm:grid-cols-2"><input value={item.customizationNotes} onChange={(event) => updateItem(item.key, { customizationNotes: event.target.value })} placeholder="Personalização / especificações" className="field h-9 min-h-9 text-xs" /><label><span className="sr-only">Entrega prometida</span><input value={item.promisedOn} onChange={(event) => updateItem(item.key, { promisedOn: event.target.value })} type="date" title="Entrega prometida" className="field h-9 min-h-9 text-xs" /></label></div> : null}{item.kind === "commercial_representative" ? <label className="mt-2 block"><span className="mb-1 block text-[11px] font-medium text-white/50">Comissão desta venda (%)</span><input value={item.commissionPercent} onChange={(event) => updateItem(item.key, { commissionPercent: Math.max(0, Math.min(100, Number(event.target.value) || 0)) })} type="number" min="0" max="100" step="0.01" className="field h-9 min-h-9 text-xs" /></label> : null}</div>
            <label><span className="mb-1 block text-[11px] font-medium text-white/50 lg:hidden">Quantidade</span><span className="flex h-11 items-center border border-white/[0.1]"><button type="button" aria-label={`Reduzir quantidade de ${item.name}`} onClick={() => updateItem(item.key, { quantity: Math.max(1, item.quantity - 1) })} className="grid h-full w-9 place-items-center text-white/48"><Minus size={13} /></button><input aria-label={`Quantidade de ${item.name}`} value={item.quantity} onChange={(event) => updateItem(item.key, { quantity: Math.max(1, Number(event.target.value) || 1) })} type="number" min="1" className="h-full min-h-0 border-y-0 px-1 text-center text-sm" /><button type="button" aria-label={`Aumentar quantidade de ${item.name}`} onClick={() => updateItem(item.key, { quantity: item.quantity + 1 })} className="grid h-full w-9 place-items-center text-white/48"><Plus size={13} /></button></span></label>
            <label><span className="mb-1 block text-[11px] font-medium text-white/50 lg:hidden">Preço</span><input aria-label={`Preço de ${item.name}`} value={centsToInput(item.unitPriceCents)} onChange={(event) => updateItem(item.key, { unitPriceCents: inputToCents(event.target.value) })} inputMode="decimal" className="field text-sm" /></label>
            {usesWarranties ? <label><span className="mb-1 block text-[11px] font-medium text-white/50 lg:hidden">Garantia em dias</span><span className="sr-only">Garantia em dias</span><input value={item.warrantyDays} onChange={(event) => updateItem(item.key, { warrantyDays: Math.max(0, Number(event.target.value) || 0) })} type="number" min="0" max="3650" className="field text-sm" /></label> : <span className="hidden lg:block" />}
            <strong className="text-sm font-semibold tabular-nums text-white/76"><span className="mb-1 block text-[11px] font-medium text-white/50 lg:hidden">Subtotal</span>{formatMoney(item.quantity * item.unitPriceCents)}</strong>
            <button type="button" onClick={() => removeItem(item.key)} aria-label={`Remover ${item.name}`} className="grid size-10 place-items-center text-white/32 hover:text-[#fb7767]"><Trash2 size={15} /></button>
          </div>)}</div>
        </div>}
      </section>
...
function TotalRow({ label, value }: { label: string; value: string }) { return <div className="flex items-center justify-between gap-4 py-2"><dt className="text-white/44">{label}</dt><dd className="font-semibold tabular-nums text-white/72">{value}</dd></div>; }
