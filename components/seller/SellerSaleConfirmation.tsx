  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <section className="panel">
        <header className="flex flex-col gap-3 border-b border-od-border p-4 sm:flex-row sm:items-center sm:justify-between">
          <div><h2 className="text-sm font-semibold text-od-text">Itens da venda</h2><p className="mt-1 text-xs text-od-text-3">Busque no catálogo ou cadastre o que está vendendo agora.</p></div>
        ...
        <div className="relative border-b border-od-border p-4">
          ...
          {matches.length > 0 ? <div className="glass absolute left-4 right-4 top-[64px] z-[var(--z-dropdown)] max-w-2xl">
            {matches.map((option) => <button type="button" key={option.key} onClick={() => addCatalogOption(option)} className="grid min-h-14 w-full grid-cols-[minmax(0,1fr)_7rem_6rem] items-center gap-3 border-b border-od-border px-3 text-left last:border-b-0 hover:bg-white/[0.04]"><span className="min-w-0"><strong className="block truncate text-sm font-semibold text-od-text-2">{option.name}</strong><span className="mt-1 block truncate text-xs text-od-text-3">{option.variantName || option.sku || "Produto principal"}</span></span><span className="text-sm font-medium tabular-nums text-od-text-2">{formatMoney(option.price)}</span><span className={`text-right text-xs ${option.available !== null && option.available <= 0 ? "text-[#fb7767]" : "text-od-text-3"}`}>{option.available === null ? "Livre" : `${option.available} un.`}</span></button>)}
          </div> : null}
        </div>

        {showQuick ? <div className="border-b border-od-accent/18 bg-od-accent/[0.035] p-4">
          <div className="mb-4 flex items-center gap-3"><span className="grid size-8 place-items-center border border-od-accent/20 bg-od-accent/[0.08] text-od-text-2"><PackagePlus size={16} /></span><div><h3 className="text-sm font-semibold text-od-text">Produto novo</h3><p className="mt-1 text-xs text-od-text-3">Preencha o essencial. Fotos e detalhes podem ser completados no catálogo.</p></div></div>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            {usesInventory ? <><QuickField label="Estoque inicial"><input value={quick.initialStock} onChange={(event) => setQuick({ ...quick, initialStock: Math.max(0, Number(event.target.value) || 0) })} type="number" min="0" className="field mt-1.5" /></QuickField><label className="flex min-h-11 items-center gap-2 self-end text-sm text-od-text-2"><input type="checkbox" checked={quick.trackStock} onChange={(event) => setQuick({ ...quick, trackStock: event.target.checked })} className="size-4 accent-od-accent" /> Controlar estoque</label></> : null}
            {usesWarranties ? <><QuickField label="Garantia em dias"><input value={quick.warrantyDays} onChange={(event) => setQuick({ ...quick, warrantyDays: Math.max(0, Number(event.target.value) || 0) })} type="number" min="0" max="3650" className="field mt-1.5" /></QuickField><QuickField label="Número de série"><input value={quick.serialNumber} onChange={(event) => setQuick({ ...quick, serialNumber: event.target.value })} placeholder="Opcional" className="field mt-1.5" /></QuickField><label className="flex min-h-11 items-center gap-2 self-end text-sm text-od-text-2"><input type="checkbox" checked={quick.requiresSerial} onChange={(event) => setQuick({ ...quick, requiresSerial: event.target.checked })} className="size-4 accent-od-accent" /> Exigir série</label></> : null}
          </div>
        ...
        {items.length === 0 ? <div className="flex min-h-44 flex-col items-center justify-center px-5 text-center"><Box size={23} className="text-od-text-3" /><p className="mt-3 text-sm font-semibold text-od-text-2">Nenhum item adicionado</p><p className="mt-1 text-xs text-od-text-3">A venda só será confirmada depois que os itens reais forem informados.</p></div> : <div>
          <div className="hidden grid-cols-[minmax(12rem,1.3fr)_7rem_7rem_8rem_7rem_2.5rem] border-b border-od-border px-4 py-2 text-xs font-semibold text-od-text-3 lg:grid"><span>Produto</span><span>Quantidade</span><span>Preço</span><span>Garantia</span><span>Subtotal</span><span /></div>
          <div className="divide-y divide-white/[0.08]">{items.map((item) => <div key={item.key} className="grid gap-3 px-4 py-3 lg:grid-cols-[minmax(12rem,1.3fr)_7rem_7rem_8rem_7rem_2.5rem] lg:items-center">
            ...
            <strong className="text-sm font-semibold tabular-nums text-od-text-2"><span className="mb-1 block text-xs font-medium text-od-text-3 lg:hidden">Subtotal</span>{formatMoney(item.quantity * item.unitPriceCents)}</strong>
      ...
      <section className="panel grid xl:grid-cols-[minmax(0,1fr)_330px]">
        <div className="grid gap-4 border-b border-od-border p-4 md:grid-cols-2 xl:border-b-0 xl:border-r">
      ...
      <footer className="od-chrome sticky bottom-[calc(76px+env(safe-area-inset-bottom))] z-[var(--z-sticky)] flex flex-col-reverse gap-2 border-t border-od-border py-3 sm:flex-row sm:items-center sm:justify-end md:bottom-0">
...
function TotalRow({ label, value }: { label: string; value: string }) { return <div className="flex items-center justify-between gap-4 py-2"><dt className="text-od-text-3">{label}</dt><dd className="font-semibold tabular-nums text-od-text-2">{value}</dd></div>; }
