  ...
  return (
    <div className={`shrink-0 space-y-2 ${className ?? ""}`}>
      {(pendingPdf || pdfError) && (
        <div>
          {pendingPdf && (
            <div className="flex items-center gap-2 rounded-[var(--radius-control)] bg-white/[0.04] px-2.5 py-1.5 text-xs font-semibold text-white/80">
              <IconPaperclip className="h-3.5 w-3.5 shrink-0 text-od-text-3" />
              <span className="min-w-0 flex-1 truncate">{pendingPdf.name}</span>
              <button
                type="button"
                onClick={clearPdf}
                className="grid size-11 shrink-0 place-items-center rounded-[var(--radius-control)] text-od-text-3 hover:bg-white/[0.06] hover:text-white"
      ...
      >
        ...
        <button
          type="button"
          onClick={pickPdf}
          className="grid size-11 shrink-0 place-items-center rounded-full text-od-text-3 hover:bg-white/[0.06] hover:text-white/70"
        ...
        <textarea
          ...
          className="max-h-[132px] min-w-0 flex-1 rounded-[var(--radius-control)] border-0 bg-white/[0.06] px-4 py-2.5 text-[14px] leading-snug text-white outline-none placeholder:text-od-text-3 focus:bg-white/[0.08]"
        />
        <button
          type="submit"
          onClick={(event) => {
          ...
          className="grid size-11 shrink-0 place-items-center rounded-full bg-od-accent text-white transition-colors hover:bg-brand-600 disabled:opacity-40"
        >
          {hasContent ? (
