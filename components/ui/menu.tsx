    ...
    >
        ...
        {navItems.map((item) => (
          <React.Fragment key={item.label}>
            ...
            <motion.div variants={itemVariants}>
              <Link href={item.href} className="group flex min-h-10 items-center border-l border-transparent px-3 text-[11px] font-medium text-white/36 transition-colors hover:border-od-accent hover:bg-white/[0.02] hover:text-white/70">
                <span className="mr-3 grid size-4 place-items-center text-white/26 group-hover:text-od-text/75">{item.icon}</span>
      ...
      <motion.div variants={itemVariants} className="border-t border-white/[0.07] pt-3">
        <button type="button" onClick={logoutItem.onClick} className="group flex min-h-10 w-full items-center px-3 text-[11px] font-medium text-white/34 transition-colors hover:text-white/68">
          <span className="mr-3 grid size-4 place-items-center text-white/25 group-hover:text-od-text/70">{logoutItem.icon}</span>
