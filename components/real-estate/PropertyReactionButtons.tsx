    ...
    <div className="flex flex-wrap gap-2">
      {OPTIONS.map((option) => (
        <button
          ...
          className="press-sm min-h-11 rounded-[var(--radius-control)] border border-line bg-surface px-3 py-2 text-xs font-semibold text-ink-soft hover:border-brand-300 hover:bg-brand-50 hover:text-brand-800 disabled:opacity-50"
        >
          {option.label}
        </button>
      ))}
