...
export function PropertyPicker({
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        <p className="mt-1.5 rounded-[var(--radius-inner)] border border-line px-3 py-6 text-center text-sm font-medium text-ink-muted">
          Nenhum imóvel ativo encontrado.
        </p>
      ) : (
        <div className="mt-1.5 max-h-80 divide-y divide-line overflow-y-auto rounded-[var(--radius-inner)] border border-line">
          {filtered.map((property) => (
            <label key={property.id} className="flex cursor-pointer items-center gap-3 px-3 py-2.5 hover:bg-surface-2">
              ...
              {property.cover_url ? (
                ...
                <img src={property.cover_url} alt={`Foto de capa de ${property.title}`} className="h-10 w-10 shrink-0 rounded-[var(--radius-inner)] border border-line object-cover" />
              ) : (
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-[var(--radius-inner)] border border-line bg-surface-2 text-ink-muted/60">
