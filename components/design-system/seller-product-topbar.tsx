...
export function SellerProductTopbar({ initials, reminderCount }: { initials: string; reminderCount: number }) {
  ...
  return (
    <header className="sticky top-0 z-40 flex min-h-14 items-center border-b border-white/[0.05] bg-[#151419] px-4 md:px-8">
      ...
      <form onSubmit={search} role="search" className="ml-4 hidden min-w-0 max-w-[340px] flex-1 items-center gap-2 rounded bg-[#1e1d22] px-3 py-1.5 focus-within:ring-2 focus-within:ring-violet-400/30 md:ml-0 md:flex">
        <Search size={14} className="shrink-0 text-white/40" />
        <input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Buscar cliente ou venda" placeholder="Buscar cliente ou venda" className="!min-h-0 !border-0 !bg-transparent !p-0 text-[13px] !shadow-none outline-none placeholder:text-white/45" />
      </form>
      <div className="ml-auto flex items-center gap-0.5">
        ...
        <Link href="/painel/tarefas" aria-label="Ver lembretes" className="relative grid size-11 place-items-center rounded-lg text-white/45 hover:bg-white/[0.05] hover:text-white">
          <Bell size={16} />
          {reminderCount > 0 ? <span className="absolute right-1.5 top-1.5 grid min-h-4 min-w-4 place-items-center rounded-full bg-[#fb7767] px-1 text-[9px] font-bold text-white">{Math.min(reminderCount, 99)}</span> : null}
        </Link>
