  ...
  return (
    <header className="sticky top-0 z-40 flex h-14 items-center justify-between gap-3 border-b border-white/[0.05] bg-[#171320]/96 px-4 backdrop-blur-md md:px-6">
      ...
      <div className="flex shrink-0 items-center gap-1">
        <Link href="/painel/assistente" aria-label="Abrir conversa com o Tim" className="grid size-11 place-items-center rounded-md text-od-text-3 hover:bg-white/[0.05] hover:text-od-text"><Bot size={17} /></Link>
        <Link href="/painel/tarefas" aria-label="Ver lembretes" className="relative grid size-11 place-items-center rounded-md text-od-text-3 hover:bg-white/[0.05] hover:text-white"><Bell size={17} /><span className="absolute right-2.5 top-2.5 size-1.5 rounded-full bg-od-accent" /></Link>
        <Link href="/painel/configuracoes" aria-label="Abrir sua conta" className="ml-1 grid size-8 place-items-center rounded-full bg-white/[0.08] text-xs font-semibold text-white/75">{initials}</Link>
      </div>
    </header>
  );
}
