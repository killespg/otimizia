    ...
    <aside className={`relative sticky top-0 z-50 hidden h-screen shrink-0 flex-col border-r border-white/[0.06] bg-[#0f0d11] md:flex ${collapsed ? "w-16" : ""}`} style={collapsed ? undefined : { width: sidebarWidth }} aria-label="Navegação imobiliária">
      ...
      <nav className="flex-1 overflow-y-auto px-2">
        {groups.map((group) => <section key={group.label || "overview"} className="mb-0 p-2">
          ...
          <div>{group.items.map((item, index) => <Fragment key={item.href + item.label}><Item item={item} />{group.label === "" && index === 0 && !collapsed && pathname === "/painel/imoveis/dashboard" ? <div className="mx-3.5 flex translate-x-px flex-col gap-1 border-l border-white/[0.08] px-2.5 py-0.5"><Link href="/painel/imoveis/dashboard" className="flex h-7 -translate-x-px items-center rounded bg-white/[0.055] px-2 text-sm font-medium text-white">Minha operação</Link><Link href="/painel/imoveis/comissoes" className="flex h-7 -translate-x-px items-center rounded px-2 text-sm text-white/42 hover:text-white">Metas e comissões</Link></div> : null}</Fragment>)}</div>
        </section>)}
      </nav>

      <footer className="border-t border-white/[0.06] p-2">
        <Link href="/painel/configuracoes" prefetch={true} className={`flex min-h-9 items-center rounded text-sm text-white/58 hover:bg-white/[0.05] hover:text-white ${collapsed ? "justify-center" : "gap-2 px-2"}`}><Settings size={16} />{!collapsed ? <span>Configurações</span> : null}</Link>
