    ...
    <section className="overflow-hidden rounded-xl border border-od-border bg-od-surface">
      <div className="flex flex-col gap-3 border-b border-od-border px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex rounded-md border border-od-border bg-od-muted-surface p-0.5">
          <button type="button" onClick={() => setView("review")} className={`min-h-8 rounded px-3 text-xs font-semibold ${view === "review" ? "bg-od-accent-tint text-violet-200" : "text-od-text-2 hover:text-od-text"}`}>Para revisar</button>
          <button type="button" onClick={() => setView("all")} className={`min-h-8 rounded px-3 text-xs font-semibold ${view === "all" ? "bg-od-accent-tint text-violet-200" : "text-od-text-2 hover:text-od-text"}`}>Todos</button>
        </div>
      </div>
      ...
      {visible.length ? (
              ...
              <article key={item.id} className="grid gap-4 px-5 py-4 transition-colors hover:bg-white/[0.025] md:grid-cols-[minmax(0,1.3fr)_minmax(180px,.8fr)_auto] md:items-center">
                <Link href={item.case_id ? `/painel/juridico/processos/${item.case_id}` : "/painel/juridico/consulta"} className="group min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`grid size-8 shrink-0 place-items-center rounded-xl ${unread ? "bg-violet-400/10 text-violet-300" : "bg-white/[0.05] text-white/40"}`}>
                      {unread ? <RefreshCw size={14} /> : <Check size={14} />}
                    </span>
                    <span className="min-w-0">
                      <strong className="block truncate text-[13px] font-semibold text-od-text group-hover:text-violet-200">{item.label || processNumber(item.case_number)}</strong>
                ...
                <div className="flex items-center justify-end gap-2">
                  {unread ? (
                    <button type="button" disabled={saving === item.id} onClick={() => markSeen(item.id)} className="inline-flex min-h-9 items-center gap-2 rounded-md border border-od-border px-3 text-[11px] font-semibold text-violet-300 hover:border-od-border-hover hover:bg-white/[0.03] disabled:opacity-50">
        ...
        <div className="px-6 py-16 text-center">
          <FileSearch className="mx-auto size-7 text-violet-300" />
